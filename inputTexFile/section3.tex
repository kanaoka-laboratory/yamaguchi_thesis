...
401,971個のAPKを展開し、得られたsmaliファイルと、4,324個のAndroid DevelopersのAPIリファレンスに記載されているAPIより取得した暗号・セキュリティに関するクラスが持つメソッドのリストを使用し、
暗号で用いられるメソッド名や特徴のある用語によるフィルタリングアルゴリズムが指定可能な代表的箇所の抽出やAPIの利用傾向分析の調査が行われた。
%メソッドリスト表いれるべき？？
本研究と関連高い調査結果を抜粋し下記に示す。


\subsubsection{河合らの調査結果}
\begin{itemize}
...
河合による先行研究ではこの上記のような結果が得られたが、 
河合は暗号利用動向の網羅的調査のために今後これらの更なる調査、分析のために
APIリファレンスに記載されているAPI以外のAPIの調査も必要であると考察した。
